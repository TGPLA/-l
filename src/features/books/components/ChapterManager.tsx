          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '0.75rem', maxWidth: '42rem', width: '100%', maxHeight: '90vh', overflowY: 'auto', padding: '1.5rem' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
                ...
                <button onClick={editingChapter ? handleUpdateChapter : handleAddChapter} disabled={!title.trim() || !content.trim() || loading} style={{ padding: '0.5rem 1rem', border: 'none', borderRadius: '0.5rem', backgroundColor: !title.trim() || !content.trim() || loading ? '#9ca3af' : '#3b82f6', color: '#ffffff', cursor: !title.trim() || !content.trim() || loading ? 'not-allowed' : 'pointer' }}>
                  {loading ? '保存中...' : '保存'}
                </button>
              </div>
            </div>
